...
- Eröffnungsfoto: `<img class="photo" src="img/…png">` rechts, `<div class="photo-text">` links, Fußnote `image: ai-generated (<modell>)`; Bild in 3840x2160 mit schwarzem Grund, erzeugt mit `python <stagekit>/tools/image.py "<beschreibung>" img/<name>.png --size 3840x2160`.
- **Bilder werden erzeugt, nicht gesucht — und zwar großzügig.** `tools/image.py` schreibt aus einer Beschreibung ein Bild (gpt-image-2). Das gilt nicht nur für die Eröffnung: Wo ein Symbolfoto, ein freigestelltes Objekt oder eine Szene eine Aussage trägt, ist ein erzeugtes Bild der Normalfall. Es passt in Format, Farbklima und Bildsprache zum Satz, es kostet Sekunden, und die Rechtefrage stellt sich nicht. Ein Stockfoto oder ein Bild aus der Suche kann das alles nicht. **Also lieber ein Bild zu viel vorschlagen als eines zu wenig** — besonders für die Eröffnung, für einen Themenwechsel und für den Schluss.
- **Ein Bildauftrag wird ausformuliert, nicht gestichwortet.** Motiv, Blickwinkel, Licht, Stimmung, und für Folienfotos ausdrücklich der schwarze Hintergrund, damit das Bild nahtlos in die Folie übergeht. Folienfotos mit `--size 3840x2160`, freigestellte Objekte mit `--transparent`. Die Fußnote `image: ai-generated (<modell>)` gehört auf jede Folie mit einem erzeugten Bild.
...
- **Das PDF ist Vektor, mit einer Ausnahme.** `export.py` lässt Chrome die Druckansicht (`?print`) direkt nach PDF drucken: Text bleibt Text, Zeichnungen bleiben SVG, nichts wird beim Zoomen pixelig, und die Datei ist rund ein Zehntel so groß. **Eingebettete Demonstratoren drucken nicht mit** — ein `<iframe>` bleibt im PDF leer. Trägt ein Satz Demonstratoren, die im PDF zu sehen sein müssen, `--pdf-raster` nehmen: Das baut das PDF wie früher aus den Frame-PNGs, also aus Bildern der Folien. Für Fotofolien ist beides gleich gut, ein Foto ist ohnehin ein Bild.
...
- **Das IPO-Modell kehrt wieder.** Wo ein Konzept Daten verarbeitet (packen, verschlüsseln, filtern, prüfen), zeigt eine Folie am Anfang die Verarbeitung als Kasten mit Eingabe und Ausgabe, immer über `d.ipo(y, input, processing, output, {level, active, monoBox})`, mit derselben Geometrie wie in Deck 03. Das Modell ist die zentrale Denkform des Kurses und soll so oft wie möglich auf den Plan (Regel vom 14.09.2026).
...
- **Demonstratoren auf Folien** laden mit `?embed=1` (Kopfzeile, Fußzeile und Hinweise ausgeblendet) und ihren Startparametern in der Adresse, in `.embed.z125` oder `.z150`, damit die Bedienelemente auf der Leinwand lesbar sind. Die Folie trägt nur die kleine überschrift `try it: …`. **Im PDF bleibt ein solcher Rahmen leer** (ein `<iframe>` druckt nicht mit); wer das PDF weitergibt, exportiert den Satz mit `--pdf-raster`.
...
d.ipo(y, input, processing, output, {level, active, monoBox})  // eine IPO-Zeile wie in Deck 03: Eingabe, Kasten, Ausgabe
